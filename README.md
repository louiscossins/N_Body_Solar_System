...
To reproduce the analysis, run the corresponding Python scripts in the order listed below.

| Script Name | Simulation Subject | Ephemeris / Setup Details | Approx. Runtime |
| :--- | :--- | :--- | :--- |
| `Test case.py` | 2D Stable Binary System | 2 equal-mass bodies ($1\,M_\odot$), circular binary orbit ($T = 4.38\text{ hrs}$) | $< 1\text{ min}$ |
| `Burrau's Problem.py` | Pythogoras 3-Body Problem | Masses: $3, 4, 5$. Evolution & escape dynamics over $T=70$ units | $\sim 1\text{ min}$ |
| `solar system code.py` | 10-Body Solar System | Target: Sun, Mercury–Pluto<br>Observer: `@500` (Sun center)<br>Epoch: `2024-Nov-29 00:00:00.0000 TDB` | $< 15\text{ mins}$ |
| `Sun plot over 70 years.py` | Solar Barycenter Motion | Historical tracking of Solar System barycenter vs. Sun from `1944-Dec-12` | $< 15\text{ mins}$ |
| `barycenter plot.py` | Planetary Removal Dynamics | Runs 4 variations (Full System, No Jupiter, No Saturn, No Jupiter/Saturn) over 250 years | $30 - 45\text{ mins}$ |
| `Barycenter plot 2.py` | Barycenter Distance & FFT | 500-year evolution and spectral frequency decomposition of barycentric motion | $30 - 45\text{ mins}$ |
| `Earth to Mars close encounter.py` | Earth–Mars Distance Spectra | 250-year distance tracking; FFT extracts synodic period ($2.14\text{ yrs}$) and resonance ($15.62\text{ yrs}$) | $< 10\text{ mins}$ |
| `phobos and deimos - Copy.py` | Martian System Dynamics | Target: Sun, Mercury, Venus, Earth, Mars, Phobos, Deimos, Jupiter, Saturn<br>Epoch: `2024-Nov-29` | $< 15\text{ mins}$ |
