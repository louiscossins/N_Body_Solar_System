...
#bary centre with fixed sun
fig = plt.figure(figsize=(16, 10))
ax_solar_sun_var = plt.axes([0.06,0.2,0.4,0.5])
ax_solar_sun_f = plt.axes([0.54,0.2,0.4,0.5])
moment_x = np.zeros(N)
moment_y = np.zeros(N)
moment_z = np.zeros(N)
...
rel_x_cm = x_barycentre - x_pos[0,:]
rel_y_cm = y_barycentre - y_pos[0,:]
rel_z_cm = z_barycentre - z_pos[0,:]
r_cm = np.sqrt(rel_x_cm**2 + rel_y_cm**2 + rel_z_cm**2)
ax_solar_sun_var.plot(Years,r_cm/m_AU,color = 'black')
#The equatorial radius of sun is 696,340 km
R_sun = (696340e3)*np.ones(len(r_cm))
ax_solar_sun_var.plot(Years,R_sun/m_AU,color = 'red',linestyle = 'dashed')
#Frequency space
r_cm_centred = r_cm - np.mean(r_cm)
f = fftshift(fftfreq(N,Time/N))[int(N/2 - 1):int(N-1)]
r_cm_f = (fftshift(fft(r_cm_centred)))
Powercm = ((np.abs(r_cm_f)**2)/(N**2))[int(N/2 - 1):int(N-1)]
ax_solar_sun_f.plot(f,Powercm,color = 'black')
...
rel_x_cm_nj = x_barycentre - x_pos[0,:]
rel_y_cm_nj = y_barycentre - y_pos[0,:]
rel_z_cm_nj = z_barycentre - z_pos[0,:]
r_cm_nj = np.sqrt(rel_x_cm_nj**2 + rel_y_cm_nj**2 + rel_z_cm_nj**2)
ax_solar_sun_var.plot(Years,r_cm_nj/m_AU,color = 'blue')
#Frequency space
r_cmnj_centred = r_cm_nj - np.mean(r_cm_nj)
f = fftshift(fftfreq(N,Time/N))[int(N/2 - 1):int(N-1)]
r_cmnj_f = (fftshift(fft(r_cmnj_centred)))
Powercmnj = ((np.abs(r_cmnj_f)**2)/(N**2))[int(N/2 - 1):int(N-1)]
ax_solar_sun_f.plot(f,Powercmnj,color = 'blue')
...
rel_x_cm_ns = x_barycentre - x_pos[0,:]
rel_y_cm_ns = y_barycentre - y_pos[0,:]
rel_z_cm_ns = z_barycentre - z_pos[0,:]
r_cm_ns = np.sqrt(rel_x_cm_ns**2 + rel_y_cm_ns**2 + rel_z_cm_ns**2)
ax_solar_sun_var.plot(Years,r_cm_ns/m_AU,color = 'orange')
#Frequency space
r_cmns_centred = r_cm_ns - np.mean(r_cm_ns)
f = fftshift(fftfreq(N,Time/N))[int(N/2 - 1):int(N-1)]
r_cmns_f = (fftshift(fft(r_cmns_centred)))
Powercmns = ((np.abs(r_cmns_f)**2)/(N**2))[int(N/2 - 1):int(N-1)]
ax_solar_sun_f.plot(f,Powercmns,color = 'orange')
...
rel_x_cm_nsj = x_barycentre - x_pos[0,:]
rel_y_cm_nsj = y_barycentre - y_pos[0,:]
rel_z_cm_nsj = z_barycentre - z_pos[0,:]
r_cm_nsj = np.sqrt(rel_x_cm_nsj**2 + rel_y_cm_nsj**2 + rel_z_cm_nsj**2)
ax_solar_sun_var.plot(Years,r_cm_nsj/m_AU,color = 'green')


ax_solar_sun_var.legend(labels = ['bary centre', 'Radius of sun,R = 696,240 km','bary centre with jupiter removed','bary centre with saturn removed','bary centre with jupiter and saturn removed'],loc = 'lower left')

ax_solar_sun_var.set_xlabel('T[yr]')
ax_solar_sun_var.set_ylabel('d[AU]')
ax_solar_sun_var.set_xlim(Years[0],Years[-1])
ax_solar_sun_var.grid(True)

#Frequency space
r_cmnsj_centred = r_cm_nsj - np.mean(r_cm_nsj)
f = fftshift(fftfreq(N,Time/N))[int(N/2 - 1):int(N-1)]
r_cmnsj_f = (fftshift(fft(r_cmnsj_centred)))
Powercmnsj = ((np.abs(r_cmnsj_f)**2)/(N**2))[int(N/2 - 1):int(N-1)]
ax_solar_sun_f.plot(f,Powercmnsj,color = 'green')
ax_solar_sun_f.set_xlim(0,2e-8)
ax_solar_sun_f.set_ylim(1e9,1e17)
ax_solar_sun_f.set_yscale("log")
ax_solar_sun_f.set_xlabel('frequency[Hz]')
ax_solar_sun_f.set_ylabel('amplitude^2/N^2')
ax_solar_sun_f.grid(True)